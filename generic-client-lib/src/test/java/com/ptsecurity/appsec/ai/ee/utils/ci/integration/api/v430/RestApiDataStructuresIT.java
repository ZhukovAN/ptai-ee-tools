...
@Slf4j
@DisplayName("Test PT AI 4.3 REST API data structures")
@Tag("development")
@Environment(enabledFor = { V430 })
public class RestApiDataStructuresIT extends BaseClientIT {

    @SneakyThrows
    protected void generateData(@NonNull final Path destination, @NonNull final Project project, @NonNull final Consumer<UnifiedAiProjScanSettings> modifySettings) {
        RawData rawData = RawData.builder()
                .fileName(UUID.randomUUID() + ".json")
                .build();

        UnifiedAiProjScanSettings settings = project.getSettings().clone();
        ...
        com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v430.ApiClient client = new ApiClient(connectionSettings);
        // Initialize all API clients with URL, timeouts, SSL settings etc.
        client.init();
        client.authenticate();

        Path jsons = destination.resolve("v430").resolve("json");
        jsons.toFile().mkdirs();
