...
import com.ptsecurity.appsec.ai.ee.server.v460.api.model.ScanResultModel;
import com.ptsecurity.appsec.ai.ee.server.v460.api.model.ScanSettingsModel;
import com.ptsecurity.appsec.ai.ee.server.v460.api.model.VulnerabilityModel;
...
public class ConverterTest extends BaseTest {
    @SneakyThrows
    public ScanResult generateScanResultV460(@NonNull final String fileName) {
        ObjectMapper mapper = createObjectMapper();
        log.trace("Read scan results");
        String scanResultStr = getResourceString("v460/json/scanResult/" + fileName + ".json");
        ScanResultModel scanResult = mapper.readValue(scanResultStr, ScanResultModel.class);
        log.trace("Read scan issues");
        String scanIssuesStr = getResourceString("v460/json/issuesModel/" + fileName + ".json");
        ...
        for (Reports.Locale locale : Reports.Locale.values()) {
            Path issuesFile = extractResourceFile("v460/json/issuesModel/" + fileName + "." + locale.getLocale().getLanguage() + ".json.7z");
        ...
        @NonNull final ScanSettingsModel scanSettings = mapper.readValue(
                getResourceString("v460/json/scanSettings/" + fileName + ".json"),
                ScanSettingsModel.class
        );
        Map<ServerVersionTasks.Component, String> versions = new HashMap<>();
        versions.put(ServerVersionTasks.Component.AIE, "4.6.0.29359");
        versions.put(ServerVersionTasks.Component.AIC, "4.6.0.29359");

        String projectName = StringUtils.substringBefore(fileName, ".");

        return com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v460.converters.IssuesConverter.convert(projectName, scanResult, issues, issuesHeadersFiles, scanSettings, "https://ptai460.domain.org", versions);
    }

    @Test
    @DisplayName("Convert PT AI 4.6 scan results")
    @SneakyThrows
    public void generateScanResults() {
        try (TempFile destination = TempFile.createFolder()) {
            Path scanResults460 = destination.toPath().resolve("result").resolve("v460");
            assertTrue(scanResults460.toFile().mkdirs());

            for (ProjectTemplate.ID templateId : ProjectTemplate.ID.values()) {
                ProjectTemplate projectTemplate = getTemplate(templateId);
                ScanResult scanResult = generateScanResultV460(projectTemplate.getName());
                String json = BaseJsonHelper.minimize(scanResult);
                packData7Zip(scanResults460.resolve(projectTemplate.getName() + ".json.7z"), json.getBytes(StandardCharsets.UTF_8));
            }
