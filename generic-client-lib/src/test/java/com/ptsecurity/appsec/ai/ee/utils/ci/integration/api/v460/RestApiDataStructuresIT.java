...
import com.ptsecurity.appsec.ai.ee.server.v460.api.ApiResponse;
...
@Slf4j
@DisplayName("Test PT AI 4.6 REST API data structures")
@Tag("development")
...
public class RestApiDataStructuresIT extends BaseClientIT {
    ...
    protected void generateData(@NonNull final Path destination, @NonNull final ProjectTemplate.ID templateId, @NonNull final Consumer<UnifiedAiProjScanSettings> modifySettings) {
        ...
        com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v460.ApiClient client = new ApiClient(connectionSettings);
        // Initialize all API clients with URL, timeouts, SSL settings etc.
        client.init();
        client.authenticate();

        Path jsons = destination.resolve("v460").resolve("json");
        jsons.toFile().mkdirs();
