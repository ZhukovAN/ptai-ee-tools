...
public class RestApiDataStructuresIT extends BaseClientIT {
    ...
    protected void generateData(@NonNull final Path destination, @NonNull final BaseAstIT.Project project, @NonNull final Consumer<JsonSettingsTestHelper> modifySettings) {
        if (Connection.Version.V36 != CONNECTION().getVersion()) return;
        ...
        Path jsons = destination.resolve("v36").resolve("json");
        jsons.toFile().mkdirs();
        ...
        for (Reports.Locale locale : Reports.Locale.values()) {
            ...
            File issuesModelFile = client.getProjectsApi().apiProjectsProjectIdScanResultsScanResultIdIssuesGet(scanResult.getProjectId(), scanResult.getId(), locale.getCode());
