...
import com.ptsecurity.appsec.ai.ee.server.v44x.api.model.ScanResultModel;
import com.ptsecurity.appsec.ai.ee.server.v44x.api.model.ScanSettingsModel;
import com.ptsecurity.appsec.ai.ee.server.v44x.api.model.VulnerabilityModel;
...
public class ConverterTest extends BaseTest {
    @SneakyThrows
    public ScanResult generateScanResultV44x(@NonNull final String fileName) {
        ObjectMapper mapper = createObjectMapper();
        log.trace("Read scan results");
        String scanResultStr = getResourceString("v44x/json/scanResult/" + fileName + ".json");
        ScanResultModel scanResult = mapper.readValue(scanResultStr, ScanResultModel.class);
        log.trace("Read scan issues");
        String scanIssuesStr = getResourceString("v44x/json/issuesModel/" + fileName + ".json");
        ...
        for (Reports.Locale locale : Reports.Locale.values()) {
            Path issuesFile = extractResourceFile("v44x/json/issuesModel/" + fileName + "." + locale.getLocale().getLanguage() + ".json.7z");
        ...
        @NonNull final ScanSettingsModel scanSettings = mapper.readValue(
                getResourceString("v44x/json/scanSettings/" + fileName + ".json"),
                ScanSettingsModel.class
        );
        Map<ServerVersionTasks.Component, String> versions = new HashMap<>();
        versions.put(ServerVersionTasks.Component.AIE, "4.4.1.23997");
        versions.put(ServerVersionTasks.Component.AIC, "4.4.1.23997");

        String projectName = StringUtils.substringBefore(fileName, ".");

        return com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v44x.converters.IssuesConverter.convert(projectName, scanResult, issues, issuesHeadersFiles, scanSettings, "https://ptai44x.domain.org", versions);
    }

    @Test
    @DisplayName("Convert PT AI 4.4 scan results")
    @SneakyThrows
    public void generateScanResults() {
        try (TempFile destination = TempFile.createFolder()) {
            Path scanResults44x = destination.toPath().resolve("result").resolve("v44x");
            assertTrue(scanResults44x.toFile().mkdirs());

            for (ProjectTemplate.ID templateId : ProjectTemplate.ID.values()) {
                ProjectTemplate projectTemplate = getTemplate(templateId);
                ScanResult scanResult = generateScanResultV44x(projectTemplate.getName());
                String json = BaseJsonHelper.minimize(scanResult);
                packData7Zip(scanResults44x.resolve(projectTemplate.getName() + ".json.7z"), json.getBytes(StandardCharsets.UTF_8));
            }
