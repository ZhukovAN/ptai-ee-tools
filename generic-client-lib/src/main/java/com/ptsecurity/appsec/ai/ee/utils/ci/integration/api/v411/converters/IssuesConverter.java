...
import com.ptsecurity.appsec.ai.ee.server.v411.projectmanagement.JSON;
...
public class IssuesConverter {
    ...
    private static final Map<V411VulnerabilityIssueScanMode, VulnerabilityIssue.ScanMode> SCAN_MODE_MAP = new HashMap<>();
    private static final Map<PolicyState, Policy.State> POLICY_STATE_MAP = new HashMap<>();
    private static final Map<V411ProgrammingLanguage, ScanResult.ScanSettings.Language> LANGUAGE_MAP = new HashMap<>();
    private static final Map<Stage, ScanResult.State> STATE_MAP = new HashMap<>();
    private static final Map<ScanResult.ScanSettings.Language, V411ProgrammingLanguage> REVERSE_LANGUAGE_MAP = new HashMap<>();
    ...
    static {
        ...
        SCAN_MODE_MAP.put(V411VulnerabilityIssueScanMode.FromEntryPoint, VulnerabilityIssue.ScanMode.FROM_ENTRYPOINT);
        SCAN_MODE_MAP.put(V411VulnerabilityIssueScanMode.FromPublicProtected, VulnerabilityIssue.ScanMode.FROM_PUBLICPROTECTED);
        ...
        LANGUAGE_MAP.put(V411ProgrammingLanguage.JAVA, ScanResult.ScanSettings.Language.JAVA);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.PHP, ScanResult.ScanSettings.Language.PHP);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.CSHARP, ScanResult.ScanSettings.Language.CSHARP);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.VB, ScanResult.ScanSettings.Language.VB);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.GO, ScanResult.ScanSettings.Language.GO);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.CPLUSPLUS, ScanResult.ScanSettings.Language.CPP);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.PYTHON, ScanResult.ScanSettings.Language.PYTHON);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.PLSQL, ScanResult.ScanSettings.Language.SQL);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.JAVASCRIPT, ScanResult.ScanSettings.Language.JAVASCRIPT);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.KOTLIN, ScanResult.ScanSettings.Language.KOTLIN);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.SWIFT, ScanResult.ScanSettings.Language.SWIFT);
        LANGUAGE_MAP.put(V411ProgrammingLanguage.OBJECTIVEC, ScanResult.ScanSettings.Language.OBJECTIVEC);
        for (V411ProgrammingLanguage language : LANGUAGE_MAP.keySet())
    ...
    public static ScanResult convert(
        ...
        for (Reports.Locale locale : modelFiles.keySet()) {
            ...
            Map<String, V411IssueDescriptionModel> descriptions = localizedModel.getDescriptions();
            for (V411IssueDescriptionModel idm : descriptions.values()) {
    ...
    public static ScanBrief.Statistics convert(
            ...
            @NonNull final com.ptsecurity.appsec.ai.ee.server.v411.projectmanagement.model.ScanResultModel scanResult) {
    ...
    public static VulnerabilityIssue.Exploit convert(final V411Exploit exploit) {
    ...
    public static VulnerabilityIssue.Exploit.Parameter convert(final V411ExploitParameter parameter) {
    ...
    public static VulnerabilityIssue.BestPlaceToFix convert(final V411BestPlaceToFix bpf) {
    ...
    public static BaseSourceIssue.Place convert(final V411Place place) {
     ...
     */
    public static ScaIssue.Cvss convert(final V411CvssMetadata cvss) {
    ...
    protected static void applyMetadata(@NonNull final IssueBaseMetadata metadata, @NonNull final BaseIssue issue) {
        ...
        if (haveAccessor(metadata, "getCvss") && haveAccessor(issue, "setCvss", ScaIssue.Cvss.class)) {
            V411CvssMetadata value = on(metadata).call("getCvss").get();
    ...
    protected static List<BaseIssue> convert(
        ...
        if (IssueType.BlackBox == issueType && issueBase instanceof V411BlackBoxIssue) {
                V411BlackBoxIssue issue = (V411BlackBoxIssue) issueBase;
        ...
        } else if (IssueType.Configuration == issueType && issueBase instanceof V411ConfigurationIssue) {
            V411ConfigurationIssue issue = (V411ConfigurationIssue) issueBase;
            ...
            V411FingerprintIssue issue = (V411FingerprintIssue) issueBase;
            ...
            for (String fingerprintId : fingerprintIds) {
                ...
                setBaseFields(issue, res, dictionary);
                // As single v411FingerprintIssue may have multiple fingerprint IDs
            ...
            V411UnknownIssue issue = (V411UnknownIssue) issueBase;

            UnknownIssue res = new UnknownIssue();
            setBaseFields(issue, res, dictionary);
            return Collections.singletonList(res);
        } else if (IssueType.Vulnerability == issueType) {
            V411VulnerabilityIssue issue = (V411VulnerabilityIssue) issueBase;
            ...
            List<V411Place> entries = issue.getTaintDataEntries();
            ...
            V411WeaknessIssue issue = (V411WeaknessIssue) issueBase;
            ...
            V411YaraMatchIssue issue = (V411YaraMatchIssue) issueBase;
    ...
    @NonNull
    public static V411ProgrammingLanguage convertLanguage(@NonNull final ScanResult.ScanSettings.Language language) {
        V411ProgrammingLanguage res = REVERSE_LANGUAGE_MAP.get(language);
        return null == res ? V411ProgrammingLanguage.NONE : res;
    }
