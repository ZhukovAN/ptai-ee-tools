package com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v460;
...
import com.ptsecurity.appsec.ai.ee.server.v460.api.api.*;
import com.ptsecurity.appsec.ai.ee.server.v460.api.model.ScanAgentModel;
import com.ptsecurity.appsec.ai.ee.server.v460.auth.ApiResponse;
import com.ptsecurity.appsec.ai.ee.server.v460.auth.api.AuthApi;
import com.ptsecurity.appsec.ai.ee.server.v460.auth.model.AuthResultModel;
import com.ptsecurity.appsec.ai.ee.server.v460.auth.model.UserLoginModel;
import com.ptsecurity.appsec.ai.ee.server.v460.notifications.model.*;
...
import com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v460.tasks.GenericAstTasksImpl;
import com.ptsecurity.appsec.ai.ee.utils.ci.integration.api.v460.tasks.ServerVersionTasksImpl;
...
import static com.ptsecurity.appsec.ai.ee.server.v460.auth.model.AuthScopeType.ACCESSTOKEN;
import static com.ptsecurity.appsec.ai.ee.server.v460.auth.model.AuthScopeType.WEB;
import static com.ptsecurity.appsec.ai.ee.server.v460.notifications.model.Stage.ABORTED;
import static com.ptsecurity.appsec.ai.ee.server.v460.notifications.model.Stage.FAILED;
import static com.ptsecurity.misc.tools.helpers.CallHelper.call;

@Slf4j
@VersionRange(min = { 4, 6, 0, 0 }, max = { 4, 6, 99999, 99999 })
public class ApiClient extends AbstractApiClient {
    ...
    @Getter
    @ToString.Exclude
    protected final AuthApi authApi = new AuthApi(new com.ptsecurity.appsec.ai.ee.server.v460.auth.ApiClient());

    @Getter
    @ToString.Exclude
    protected final ProjectsApi projectsApi = new ProjectsApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());

    @Getter
    @ToString.Exclude
    protected final ReportsApi reportsApi = new ReportsApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());

    @Getter
    @ToString.Exclude
    protected final ConfigsApi configsApi = new ConfigsApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());

    @Getter
    @ToString.Exclude
    protected final LicenseApi licenseApi = new LicenseApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());

    @Getter
    @ToString.Exclude
    protected final ScanQueueApi scanQueueApi = new ScanQueueApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());

    @Getter
    @ToString.Exclude
    protected final ScanAgentApi scanAgentApi = new ScanAgentApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());

    @Getter
    @ToString.Exclude
    protected final StoreApi storeApi = new StoreApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());

    @Getter
    @ToString.Exclude
    protected final HealthCheckApi healthCheckApi = new HealthCheckApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());

    @Getter
    @ToString.Exclude
    protected final VersionApi versionApi = new VersionApi(new com.ptsecurity.appsec.ai.ee.server.v460.api.ApiClient());
    ...
    public ScanBrief.ApiVersion getApiVersion() {
        return ScanBrief.ApiVersion.V460;
    }
        ...
        connection.on("ScanProgress", (data) -> {
            ...
            if (!scanBrief.getId().equals(data.getScanResultId()))
                ...
                builder.append(Optional.of(data)
                        ...
                        .map(com.ptsecurity.appsec.ai.ee.server.v460.notifications.model.Stage::getValue)
                ...
                Optional<com.ptsecurity.appsec.ai.ee.server.v460.notifications.model.Stage> stage = Optional.of(data).map(ScanProgress::getProgress).map(ScanProgressModel::getStage);
                if (stage.isPresent()) {
